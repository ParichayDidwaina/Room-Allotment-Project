...
class Application(tk.Tk): #This is the main window
    def __init__(self,*args):
        ...
        self.frames={}
        for fram in (loginpage,rdetails,status,success,reg):
...
class loginpage(tk.Frame): #This is the login frame
    def __init__(self,parent,controller):
        ...
        def aorder():  #function to show the allotment order in a pop up window
            ...
            def _delete_window():
                try:
                    abutton.config(state='enabled')
                    popup1.destroy()
                except:
                    pass
            popup1.protocol("WM_DELETE_WINDOW",_delete_window)
        ...
        rdbutton.pack(pady=10,padx=20)
        abutton=ttk.Button(self,text="Allotment Order",command=lambda:aorder())
        abutton.pack(pady=10,padx=20)
        button=ttk.Button(self,text="Check Status",command=lambda:controller.show_frame(status))
...
class rdetails(tk.Frame): #This is the room details frame
    def __init__(self,parent,controller):
        ...
        rno=StringVar()
        def rdet(block,rno):
            b_ent.delete('0',tk.END)
            r_ent.delete('0',tk.END)
            ...
            num.pack(pady=10,padx=10,side=tk.RIGHT)
...
class status(tk.Frame): #This is the room details frame
    def __init__(self,parent,controller):
        rno=StringVar()
        def rdet(rno):
            b_ent.delete('0',tk.END)
            f=0
            rno = rno.upper()
            cur.execute("select * from room where roomid=(select roomid from student where studentid='%s')"%(rno))
            res=cur.fetchall()
            try:
                size=res[0]['beds']
                block=res[0]['block']
            ...
            if(f==0):
                num=tk.Label(rinfo,text="No of beds={}\nBLOCK={}\nROOM NO:{}\nTYPE:{}".format(size,block,roomno,rtype),font=F_FONT)
                num.pack(pady=10,padx=10,side=tk.RIGHT)
            else:
                num=tk.Label(rinfo,text="NO ROOM ALLOTED",font=F_FONT)
                num.pack(pady=10,padx=10,side=tk.BOTTOM)
                
        tk.Frame.__init__(self,parent)
        block=tk.Label(self,text="Reistration Number:",fg="#383239",bg="#a1dbcd")
        b_ent=tk.Entry(self,textvariable=rno)
        block.pack(pady=10,padx=10)
        b_ent.pack(pady=5,padx=5)
        getinfo=ttk.Button(self,text="Get Status",command=lambda: rdet(rno.get()))
        getinfo.pack(pady=7,padx=7)
        button=ttk.Button(self,text="Back to Login Page",command=lambda: controller.show_frame(loginpage))
        button.pack()
...
conn=pymysql.connect(host='localhost',user='root',password='parichay',db='project')
cur=conn.cursor(pymysql.cursors.DictCursor)
#cur.execute("update student set roomID=NULL")
#cur.execute("update room set taken=0")
conn.commit()
app= Application()
app.mainloop()
